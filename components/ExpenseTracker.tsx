...
const ExpenseTracker: React.FC<ExpenseTrackerProps> = ({ categories, expenses, lang, onAddExpense, onDeleteExpense, gender }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Confirmation Backdrop for Add (Full Screen for focus) */}
      {pendingAdd && (
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-8 text-center shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button 
                ...
                className={`py-4 ${themeBg} text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-100 transition-all active:scale-95`}
              >
      ...
      {/* Confirmation Backdrop for Delete */}
      {pendingDelete && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 animate-pop-in">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-8 text-center shadow-2xl border-2 border-rose-50">
            ...
            <p className="text-slate-500 font-medium text-sm mb-8">
              {lang === 'ar' ? 'هذا الإجراء سيقوم بإلغاء المبلغ من الفئة المحددة.' : 'Cette action déduira le montant de votre budget actuel.'}
            </p>
            <div className="grid grid-cols-2 gap-3">
              <button 
              ...
              <button 
                ...
                className="py-4 bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-rose-100 transition-all active:scale-95"
              >
        ...
        <div className="relative z-10">
          ...
          <div className="mb-6">
            <select
              ...
              className={`w-full ${themeBgDark} border-2 ${themeBorderFocus} rounded-2xl p-3.5 text-white focus:outline-none focus:border-white transition-all font-bold appearance-none cursor-pointer text-sm`}
            >
              {categories.map((cat) => (
                <option key={cat.id} value={cat.id} className={`${themeBgOption} text-white`}>
                  {cat.name[lang]}
                </option>
              ))}
            </select>
          ...
          <form onSubmit={handleSubmit} className="flex gap-2">
            <div className="relative flex-grow">
              <input
                ...
                className={`w-full ${themeBgDark} border-2 ${themeBorderFocus} rounded-2xl p-4 ${lang === 'ar' ? 'pl-14' : 'pr-14'} text-white placeholder:text-white/20 focus:outline-none focus:border-white transition-all font-black text-xl`}
                placeholder="0.00"
              />
              <span className={`absolute ${lang === 'ar' ? 'left-4' : 'right-4'} top-1/2 -translate-y-1/2 font-black text-xs uppercase ${themeTextAccent} pointer-events-none`}>
                {currencySymbol}
              </span>
            </div>
            <button
              type="submit"
              disabled={!amount}
              className={`w-16 bg-white ${themeTextBtn} rounded-2xl flex items-center justify-center shadow-lg active:scale-90 transition-transform disabled:opacity-50`}
            >
      ...
      {expenses.length > 0 && (
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
          <h3 className="font-black text-slate-900 text-xs uppercase tracking-widest mb-4">
            {lang === 'ar' ? 'المصاريف الأخيرة' : 'Dépenses Récentes'}
          </h3>
                ...
                <div key={exp.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-slate-200 transition-all group">
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
                      {category?.name[lang]} • {new Date(exp.date).toLocaleTimeString(lang === 'ar' ? 'ar-DZ' : 'fr-FR', { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
                  <button 
                    onClick={() => setPendingDelete(exp.id)}
                    className="w-8 h-8 rounded-xl bg-white border border-slate-100 text-slate-300 hover:text-rose-500 hover:border-rose-100 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
                  >
