          ...
          <div className="space-y-4">
            {currentQuestion.options.map((opt, i) => (
              <button
                ...
                className="w-full text-left p-5 rounded-[1.5rem] border-2 border-slate-50 bg-slate-50 hover:border-emerald-500 hover:bg-emerald-50 transition-all group flex items-center gap-5 rtl:text-right"
              >
                <div className="flex-shrink-0 w-12 h-12 rounded-2xl bg-white shadow-sm group-hover:bg-emerald-500 group-hover:text-white flex items-center justify-center font-black text-slate-400 transition-all text-lg">
                  {i + 1}
                </div>
          ...
          <div className="space-y-6">
            <div className="relative">
              <input
                ...
                placeholder="0"
                className="w-full p-6 bg-slate-50 rounded-3xl border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:outline-none text-3xl font-black text-center"
                autoFocus
              />
            ...
            <button
              ...
              disabled={!numInput}
              className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black text-xl hover:bg-emerald-700 disabled:opacity-50 transition-all shadow-xl shadow-emerald-100"
            >
              {t.valider}
            </button>
          </div>
        ...
        {currentQuestion.type === 'multi' && currentQuestion.options && (
          <div className="space-y-4">
              ...
              {currentQuestion.options.map((opt, i) => (
                ...
                >
                  ...
                  <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${
                    multiInput.includes(opt.value) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 bg-white'
                  }`}>
                    {multiInput.includes(opt.value) && (
            ...
            <button
              ...
              className="w-full mt-4 py-5 bg-emerald-600 text-white rounded-2xl font-black text-xl hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-100"
            >
              {t.suivant}
            </button>
          </div>
